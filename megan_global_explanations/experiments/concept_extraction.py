"""
This is the base experiment for the generation of a concept clustering from a CSV dataset and an
already pre-trained Megan model. Unlike the VGD-based experiment, this experiment loads data directly
from a CSV file and computes graph representations on-the-fly using a Processing instance.

After the model and the dataset have been loaded, the HDBSCAN algorithm will be used to find the
concept clusters in the model's latent space for each of the explanation channels.
...
# :param CSV_PATH:
#       The absolute path to the CSV file containing the data. The CSV file should have a header
...
# The parameters determine how graph representations are computed from the raw values.
...
#       MoleculeProcessing instance will be used.
...
MODEL_PATH: str = os.path.join(ASSETS_PATH, 'models', 'model.ckpt')
...
def load_processing(e: Experiment) -> ProcessingBase:
    """
    Load or create the Processing instance used for converting values to graphs.

    If PROCESSING_PATH is specified and exists, loads the processing instance from that module.
    Otherwise, creates a default MoleculeProcessing instance.

    :returns: A ProcessingBase instance for converting values to graph representations
    """
...
def filter_element(e: Experiment,
                   value: str,
                   row: dict,
                   processing: ProcessingBase) -> bool:
    """
    Filter molecular elements before graph processing.

    Filters out:
    - Mixtures/salts (SMILES containing ".")
    - Single-atom molecules
    - Invalid SMILES that RDKit cannot parse
    ...
    # Filter invalid or single-atom molecules
    try:
...
def load_dataset(e: Experiment,
    ...
    :returns: Tuple of (index_data_map, processing)
    """
            ...
            try:
                ...
                index_data_map[index] = {
                    'metadata': {
                        'index': index,
                        'graph': graph,
                        'repr': value,  # Original representation (SMILES)
                    },
    ...
    # Set node_dim, edge_dim, out_dim from first graph
...
def get_visualization(e: Experiment,
    ...
    :param metadata: The element metadata dictionary
    :returns: Path to the visualization image
    """
...
def describe_prototype(e: Experiment,
    ...
    """
    This hook takes the string representation of the prototype and the path to the visualization
    and returns a description string for the prototype.

    The default implementation returns a placeholder message. Override in child experiments
    to implement domain-specific description generation (e.g., using GPT-4).
    """
    return f'Prototype Representation: {value}\nNo description generated.'
...
def experiment(e: Experiment):
    ...
    # ~ loading the dataset
    # The dataset is loaded from the CSV file and converted to graph representations on-the-fly.

    e.log('loading dataset...')
    # :hook load_dataset:
    #       This hook is responsible for loading the dataset from the CSV path and returning it as a
    #       index_data_map. Additionally, this function sets up the experiment values "node_dim",
    #       "edge_dim" and "out_dim" based on the dataset that has been loaded.
    #       It also returns the processing object used for graph conversion.
    ...
    # For particularly large datasets we need a method to reduce the number of graphs for the clustering
    ...
    for index, graph, info, dev in zip(indices, graphs, infos, deviations):
        ...
        # Get the repr from metadata if available
    ...
    for channel_index in range(num_channels):
        ...
        for cluster in clusters:
            ...
            # Generate visualizations for cluster elements on-demand
            # IMPORTANT: This must happen BEFORE creating cluster_elements deepcopy
            # so that the image_path values are captured in the copy
            cluster_image_paths = []
            for i in cluster_indices:
                data = index_data_map[i]
                if data['image_path'] is None:
                    data['image_path'] = e.apply_hook(
                        'get_visualization',
                        index=i,
                        metadata=data['metadata']
                    )
                cluster_image_paths.append(data['image_path'])

            # Create deepcopy AFTER visualizations are generated so image_path is included
    ...
    # ~ exporting cluster centroids to JSON
    # This JSON file contains the cluster centroid information organized by channel,
    # which can be useful for downstream analysis and visualization.

    e.log('exporting cluster centroids to JSON...')
    centroids_data = {}
    for channel_index in range(e['num_channels']):
        channel_key = f'channel_{channel_index}'
        channel_clusters = [info for info in cluster_infos if info['channel_index'] == channel_index]

        centroids_data[channel_key] = []
        for info in channel_clusters:
            # Calculate average distance from cluster members to centroid (same metric as in report)
            centroid = info['centroid']
            embeddings = info['embeddings']
            centroid_distances = np.array([cosine(centroid, emb) for emb in embeddings])
            avg_distance = float(np.mean(centroid_distances))

            centroids_data[channel_key].append({
                'id': info['index'],
                'centroid': centroid.tolist(),
                'density': avg_distance,
            })

    centroids_path = os.path.join(e.path, 'cluster_centroids.json')
    with open(centroids_path, 'w') as f:
        json.dump(centroids_data, f, indent=2)
    e.log(f'saved cluster centroids to {centroids_path}')
